...
test('self-hosted subscription non-SS nodes preserve their fields including existing dialers', () => {
  ...
  assert.deepEqual(result, [{ ...before[0], 'dialer-proxy': '🛡️ 美西中转' }, ...before.slice(1)]);
  assert.notEqual(result, proxies);
  result.forEach((proxy, i) => assert.equal(proxy, proxies[i]));
});

test('self-hosted protocol restrictions use the exact subscription name, not node names', () => {
  for (const subName of [
    ...
    const proxies: ProxyNode[] = [{ name: '自建 US 落地', type: 'vless', _subName: subName }];
    const [result] = assignDialerProxy(proxies);
  ...
  for (const globals of [
    ...
    for (const platform of ['ClashMeta', 'Stash']) {
      ...
      assert.deepEqual(
        ...
        proxies.map((proxy, i) =>
          expected[i] === undefined ? proxy : { ...proxy, 'dialer-proxy': expected[i] },
        ),
      );
    }
