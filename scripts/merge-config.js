...
var __proxyConfigScript = (() => {
  ...
  function resolveConfigOptions(args) {
    ...
    if (args.noCache !== void 0 && args.noCache !== true && args.noCache !== false && args.noCache !== "true" && args.noCache !== "false") {
   ...
   */
  function createConfigRequest(url, timeout, refreshToken) {
    if (refreshToken === void 0) return { url, timeout };
    let requestUrl = url;
   ...
   */
  async function readConfigSource(url, label, timeout, { get, parseYaml }, refreshToken) {
  ...
  async function loadMergedConfig(config, args, runtime) {
    ...
    const refreshToken = noCache ? `${Date.now()}-${++refreshSequence}-${Math.random().toString(36).slice(2)}` : void 0;
